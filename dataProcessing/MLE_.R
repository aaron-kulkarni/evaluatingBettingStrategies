...
processGameState <- function(game){
  d <- game[,c("teamData:game_id", "gameState:winner", "gameState:teamHome", "gameState:teamAway")]
  names(d) <- c("gameid", "winner", "teamHome", "teamAway")
  
  d1 = d2 = d
  names(d1) <- c("gameid", "winner", "team", "opp.team")
  d1$teamHome <- d$teamHome
  names(d2) <- c("gameid", "winner", "opp.team", "team")
...
simuGamesFromElo <- function(elo, ngame.pair=2, home.court.advantage=50){
  if(F){
...
if(T){
  ...
  for(season in 2023){
    ...
    udate <- unique(dat2021$date)
    
    newdf <- NULL
    coef <- NULL
    for(i in 8:length(udate)){
      print(i)
      date <- udate[i]   ## current date for prediction
      ##past.dat <- dat2021[dat2021$date<=date,] 
      past.dat <- dat2021[dat2021$date<date,]  ## do not include today's data
