...
def selectColOdds(select_x):
    bettingOdds = pd.read_csv('../data/bettingOddsData/adj_prob_win_ALL.csv', index_col = 0, header = [0,1])['homeProbAdj']
...
def selectMLVal(select_x):
    ...
    mlval = mlval[::2]
    mlval.set_index(mlval.index.get_level_values(0).unique(), inplace=True)
...
mlval = selectMLVal(['team.elo.booker.lm', 'raptor.prob', 'opp.elo.booker.lm', 'team.elo.booker.combined', 'opp.elo.booker.combined', 'booker_odds.Pinnacle', 'elo.prob', 'predict.prob.booker', 'predict.prob.combined', 'elo.court30.prob', 'raptor.court30.prob'])
...
X_train, X_test, Y_train, Y_test = splitTrainTest([elo, perMetric, mlval], train_index, test_index)
...
clf = XGBClassifier(learning_rate = 0.03, max_depth = 6, min_child_weight = 6, n_estimators = 150)
